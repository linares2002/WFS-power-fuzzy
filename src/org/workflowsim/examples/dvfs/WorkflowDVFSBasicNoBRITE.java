...
import org.cloudbus.cloudsim.power.PowerVmAllocationPolicySimpleFuzzy;
...
public class WorkflowDVFSBasicNoBRITE
{
	...
	private static boolean fuzzyVms = false;
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            {
            	daxPath = System.getProperty("user.dir")+"/config/dax/Montage_25.xml";
            }
            else
            {
            	daxPath = args[0];
        	...
        	{
            	WorkflowSimTasksFUZZY.decodeJSONRules(args[1]);
            	GeneralParametersTasksFuzzy.initialise();
        	}
            
            if (args.length > 2 && !args[2].equals("default"))
            {
            	fuzzyVms = true;
            	WorkflowSimVmsFUZZY.decodeJSONRules(args[2]);
            	GeneralParametersVmsFuzzy.initialise();
            }
            else
            {
            	fuzzyVms = false;
    ...
    protected static WorkflowDVFSDatacenter createDatacenter(String name)
    {
        ...
        GeneralParametersTasksFuzzy.setMipsArray(mipsArray);
        ...
        try
        {
            ...
            if (fuzzyVms)
            {
	            ...
	            (
	            	name,
	            	characteristics,
	            	new PowerVmAllocationPolicySimpleFuzzy(hostList),
	            	storageList,
	            	0.01
	            );
            }
    ...
    protected static List<CondorVM> createVM(int userId, int vms)
    {
        ...
        {
        	mipsArray = GeneralParametersTasksFuzzy.getMipsArray();
